...
class MT5LiveFeed:
    ...
    def connect(self) -> bool:
        ...
        while self.connection_attempts < self.max_retries:
            try:
                ...
                logger.debug(f"MT5 connection attempt {self.connection_attempts}/{self.max_retries}")
    ...
    def get_tick(self, symbol: str) -> Optional[TickData]:
        ...
        try:
            ...
            logger.debug(f"{symbol} tick: bid={tick.bid:.5f}, ask={tick.ask:.5f}, spread={tick_data.spread:.1f}pips")
            return tick_data
    ...
    def get_symbol_info(self, symbol: str, use_cache: bool = True) -> Optional[SymbolInfo]:
        ...
        try:
            ...
            logger.debug(f"Symbol info for {symbol}: digits={info.digits}, volume_min={info.volume_min}, volume_max={info.volume_max}")
            return symbol_info
    ...
    def get_candles(
        ...
        if not self.is_connected():
            logger.warning(f"Not connected, cannot fetch candles for {symbol} {timeframe}")
            return None
        ...
        try:
            ...
            logger.debug(f"{symbol} {timeframe}: fetched {len(candle_list)} candles, latest close={candles[-1]['close']:.5f}")
            return candle_list
    ...
    def _mock_candles(self, symbol: str, timeframe: str, count: int) -> List[CandleData]:
...
if __name__ == '__main__':
    ...
    print("\n[TEST 5] Multi-Timeframe Candles")
    mtf = feed.get_multitf_candles('EURUSD', timeframes=['M1', 'M5', 'M15', 'H1'], count=3)
    if mtf:
        for tf, candles in mtf.items():
            if candles:
                print(f"{tf}: {len(candles)} candles, latest close={candles[-1].close:.5f}")
            else:
                print(f"{tf}: No data")
