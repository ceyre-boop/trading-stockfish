...
class PortfolioRiskManager:
    ...
    def can_open_position(
        ...
        projected_total = self.current_total_exposure - current_symbol_exposure + target_exposure
    ...
    def _apply_session_risk_scaling(
        ...
        risk_scale = session_modifiers.get('risk_scale', 1.0) if session_modifiers else 1.0
    ...
    def _apply_flow_risk_adjustments(
        ...
        if initiative_detected:
            if session_name == "POWER_HOUR":
                ...
                warnings.append("Initiative detected outside POWER_HOUR: caution applied")
        ...
        if vwap_distance is not None:
            ...
            if abs_distance > 0.03:  # More than 3% from VWAP
                ...
                warnings.append(f"Extreme VWAP distance ({abs_distance:.1%}): size halved")
    ...
    def _enforce_capacity_limits(
        ...
        if target_notional > notional_limit:
            ...
            warnings.append(f"Target notional ${target_notional:.0f} exceeds limit ${notional_limit:.0f}")
            allowed = False
        else:
    ...
    def evaluate_risk_with_context(
        ...
        """
        session_name = policy_decision.get('session_name', '') if policy_decision else ''
        session_modifiers = policy_decision.get('session_modifiers', {}) if policy_decision else {}
        flow_signals = policy_decision.get('flow_signals', {}) if policy_decision else {}
        ...
        if not capacity_ok:
            self._log(
                f"Capacity check FAILED for {symbol}: {cap_warnings}",
                session_name
            )
            return RiskDecision(
                action="BLOCK",
        ...
        if projected_symbol_exp > self.max_symbol_exposure:
            ...
            flow_adjusted_size, flow_adjustments, flow_warnings = self._apply_flow_risk_adjustments(
        ...
        projected_total = self.current_total_exposure - current_symbol_exp + projected_symbol_exp
        
        if projected_total > self.max_total_exposure:
            # Reduce unscaled size to fit portfolio limit
            available = self.max_total_exposure - (self.current_total_exposure - current_symbol_exp)
            reduced_size = available / max(price, 0.01)
            self._log(
        ...
        scenario_result = eval_result.get('scenario_result', None) if eval_result else None
        ...
        if scenario_result:
            scenario_scaled_size, scenario_risk_factor = self._apply_scenario_risk_scaling(
    ...
    def _apply_scenario_risk_scaling(
        ...
        prob_up = scenario_result.probability_up if hasattr(scenario_result, 'probability_up') else 0.33
        prob_down = scenario_result.probability_down if hasattr(scenario_result, 'probability_down') else 0.33
        prob_chop = scenario_result.probability_chop if hasattr(scenario_result, 'probability_chop') else 0.34
        scenario_alignment = scenario_result.regime_alignment if hasattr(scenario_result, 'regime_alignment') else 0.5
        ...
        # Strong misalignment: -20% size reduction (misalignment > 50%)
            ...
            self._log(f"Scenario misaligned (against prob {misalignment_prob:.0%}): -20% size reduction")
