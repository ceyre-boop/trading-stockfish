...
class PolicyDecision:
    ...
    regime_adjustments: Dict[str, Any] = field(default_factory=dict)  # What was adjusted
    
    # Scenario Context (v2.3)
    scenario_alignment: float = 0.0     # [0, 1] how well scenarios align with decision
    scenario_bias: str = ""             # bullish, bearish, symmetric, bullish_reversal, bearish_reversal
...
class PolicyEngine:
    ...
    def _apply_session_adjustments(
        ...
        adj = RiskConfig(**{
            k: getattr(risk_config, k) for k in [
                'max_risk_per_trade', 'max_daily_loss', 'max_position_size',
                'add_threshold', 'reduce_threshold', 'exit_threshold', 'reverse_threshold',
                'min_confidence', 'min_confidence_add', 'min_confidence_reduce',
                'cooldown_bars', 'high_vol_size_reduction', 'tight_liquidity_size_reduction',
                'enable_reverse', 'enable_add', 'skip_on_max_daily_loss'
            ]
        })
    ...
    def _apply_regime_conditioning(
        ...
        if regime_label == 'TREND':
            ...
            if action == TradingAction.ENTER_SMALL or action == TradingAction.ENTER_FULL:
                ...
                if abs(eval_score) > 0.5:
                    ...
                    self._log(f"  [TREND] Scaling up entry size: {target_size:.2f} → {adjusted_size:.2f}")
            
            elif action == TradingAction.ADD:
                # More aggressive adding in TREND
                adjusted_size = min(target_size * (1.0 + 0.10 * regime_boost), 1.0)
                self._log(f"  [TREND] Scaling up add size: {target_size:.2f} → {adjusted_size:.2f}")
            ...
            if action == TradingAction.ENTER_SMALL or action == TradingAction.ENTER_FULL:
                ...
                if abs(eval_score) > 0.6:
                    ...
                    self._log(f"  [RANGE] Reducing breakout entry size: {target_size:.2f} → {adjusted_size:.2f}")
                ...
                self._log(f"  [RANGE] Favoring REVERSE: {target_size:.2f} → {adjusted_size:.2f}")
            ...
            if action in [TradingAction.ENTER_SMALL, TradingAction.ENTER_FULL]:
                ...
                self._log(f"  [REVERSAL] Reduced entry size: {target_size:.2f} → {adjusted_size:.2f}")
    ...
    def _apply_scenario_conditioning(
        ...
        adjusted_action = action
        adjusted_size = target_size
        scenario_alignment = scenario_result.regime_alignment if hasattr(scenario_result, 'regime_alignment') else 0.0
        scenario_bias = scenario_result.scenario_bias if hasattr(scenario_result, 'scenario_bias') else ""
        
        regime_label = scenario_result.regime_label if hasattr(scenario_result, 'regime_label') else ""
        ...
        if regime_label == 'TREND':
            if prob_up > 0.55 and eval_score > 0.3:
                ...
                if action == TradingAction.ENTER_SMALL:
                    ...
                    self._log(f"  [TREND-SCENARIO] Strong UP prob {prob_up:.1%}: upgraded ENTER_SMALL → ENTER_FULL")
                elif action == TradingAction.HOLD and position_state.side == PositionSide.LONG:
                    adjusted_action = TradingAction.ADD  # Allow scaling
                    adjusted_size = min(position_state.size * 1.15, 1.0)
                    self._log(f"  [TREND-SCENARIO] Strong UP prob {prob_up:.1%}: converted HOLD → ADD")
                ...
                if action == TradingAction.ENTER_SMALL or action == TradingAction.ENTER_FULL:
                    ...
                    adjusted_size = 0.0
                    self._log(f"  [TREND-SCENARIO] Strong DOWN prob {prob_down:.1%}: blocked entry in bearish trend")
                elif action == TradingAction.ADD:
                    adjusted_action = TradingAction.REDUCE
                    adjusted_size = position_state.size * 0.5
                    self._log(f"  [TREND-SCENARIO] Strong DOWN prob {prob_down:.1%}: converted ADD → REDUCE")
        ...
        # RANGE regime: Symmetric with elevated chop
            ...
            if prob_chop > 0.50:
                ...
                if action in [TradingAction.ENTER_SMALL, TradingAction.ENTER_FULL]:
                    ...
                    self._log(f"  [RANGE-SCENARIO] High chop prob {prob_chop:.1%}: reduced entry size")
                
                elif action == TradingAction.ADD:
                    adjusted_action = TradingAction.HOLD
                    self._log(f"  [RANGE-SCENARIO] High chop prob {prob_chop:.1%}: blocked ADD in chop")
            ...
            if imbalance < 0.10 and action == TradingAction.ENTER_SMALL:
                ...
                self._log(f"  [RANGE-SCENARIO] Symmetric probabilities (imbalance {imbalance:.1%}): mean-reversion setup")
                adjusted_size = target_size * 0.9  # Slight size increase for mean-reversion confidence
        ...
        # REVERSAL regime: Reversal-biased scenarios  
            ...
            if prob_down > 0.60 and eval_score > 0.5:
                if action == TradingAction.HOLD or action == TradingAction.REDUCE:
                    ...
                    self._log(f"  [REVERSAL-SCENARIO] Strong DOWN prob {prob_down:.1%} after UP eval: REVERSE allowed")
            ...
            # Strong UP probability after DOWN eval: Allow reversal entries
                ...
                if action == TradingAction.HOLD or action == TradingAction.REDUCE:
                    adjusted_action = TradingAction.REVERSE
                    adjusted_size = target_size * 0.8  # Reduced size for reversal entry
                    self._log(f"  [REVERSAL-SCENARIO] Strong UP prob {prob_up:.1%} after DOWN eval: REVERSE allowed")
            
            # All actions in REVERSAL: Reduce size until confirmed
            else:
                adjusted_size = target_size * 0.7  # 30% size reduction
                self._log(f"  [REVERSAL-SCENARIO] Reduced size 30% pending reversal confirmation")
    ...
    def decide_action(
        ...
        risk_config.max_daily_loss_triggered = daily_loss_pct >= risk_config.max_daily_loss
        ...
        self._log(f"[{session_name}] Deciding action for eval={eval_score:.3f}, conf={confidence:.2f}, position={position_state.side.value}")
        ...
        if adjusted_risk_config.skip_on_max_daily_loss and adjusted_risk_config.max_daily_loss_triggered:
        ...
        reasoning.append(ReasoningFactor(
            "Volatility",
            f"Regime: {vol_regime.value}",
            weight=0.6
        ))
        reasoning.append(ReasoningFactor(
            "Liquidity",
            f"Regime: {liq_regime.value}",
            weight=0.6
        ))
        ...
        if position_state.side == PositionSide.FLAT:
            decision = self._decide_entry(
                eval_score, confidence, eval_zone,
                vol_regime, liq_regime, adjusted_risk_config, reasoning,
                initiative_move_detected, level_reaction_score, session_name
            )
        elif position_state.side == PositionSide.LONG:
            decision = self._decide_long(
                eval_score, confidence, eval_zone,
                vol_regime, liq_regime, position_state, adjusted_risk_config, reasoning,
                stop_run_detected, initiative_move_detected, session_name
            )
        else:  # SHORT
        ...
        vwap_distance = flow_signals.get('vwap_distance_pct', 0.0) if flow_signals else 0.0
        if abs(vwap_distance) > 2.0 and decision.action in [TradingAction.ENTER_SMALL, TradingAction.ENTER_FULL]:
            reasoning.append(ReasoningFactor(
                "FlowContext",
        ...
        round_proximity = flow_signals.get('round_level_proximity', None) if flow_signals else None
        if round_proximity is not None and round_proximity > 0.8 and decision.action in [TradingAction.ENTER_SMALL, TradingAction.ENTER_FULL]:
            reasoning.append(ReasoningFactor(
                "FlowContext",
        ...
        # 9. Enforce cooldown
        if position_state.bars_since_exit < adjusted_risk_config.cooldown_bars and position_state.bars_since_exit > 0:
            if decision.action in [TradingAction.ENTER_SMALL, TradingAction.ENTER_FULL, TradingAction.REVERSE]:
                reasoning.append(ReasoningFactor(
                    "Cooldown",
        ...
        if regime_label and regime_confidence >= 0.3:
            ...
            decision.regime_adjustments = {
                ...
                'size_adjustment': adjusted_size - decision.target_size,
            }
        
        # Apply scenario conditioning (v2.3)
        scenario_result = eval_result.get('scenario_result', None)
        if scenario_result:
            adjusted_action, adjusted_size, scenario_alignment, scenario_bias = self._apply_scenario_conditioning(
                decision.action,
                decision.target_size,
                eval_score,
                scenario_result,
            ...
            reasoning.append(ReasoningFactor(
                "Scenario",
                f"Scenario alignment={scenario_alignment:.0%}, bias={scenario_bias}",
                weight=0.7
            ))
        
        self._log(f"  → {decision.action.value} (target_size={decision.target_size:.2f}, session={session_name}, regime={regime_label})")
    ...
    def _decide_entry(
        ...
        if initiative_move_detected and session_name == 'POWER_HOUR' and eval_score > 0.2:
            reasoning.append(ReasoningFactor(
                "FlowContext",
        ...
        if eval_zone == EvaluationZone.NO_TRADE:
            reasoning.append(ReasoningFactor(
                "Decision",
                "No trade zone - insufficient signal",
                weight=1.0
            ))
            return PolicyDecision(
                action=TradingAction.DO_NOTHING,
        ...
        reasoning.append(ReasoningFactor(
            "Decision",
            f"{eval_zone.value} entry - opening position",
            weight=1.0
        ))
    ...
    def _decide_long(
        ...
        if eval_score < risk_config.exit_threshold and confidence > risk_config.min_confidence_reduce:
            reasoning.append(ReasoningFactor(
                "Decision",
        ...
        if eval_score > risk_config.add_threshold and \
           confidence > risk_config.min_confidence_add and \
           risk_config.enable_add:
            size = self._compute_position_size(
                eval_score, confidence, vol_regime, liq_regime, risk_config
            )
            reasoning.append(ReasoningFactor(
            ...
            return PolicyDecision(
                ...
                target_size=min(position_state.size + size * 0.5, risk_config.max_position_size),
                confidence=confidence,
                reasoning=reasoning
            )
        
        # Check for reduce signals (eval weakens)
        if eval_score < risk_config.reduce_threshold and \
           confidence > risk_config.min_confidence_reduce:
            reasoning.append(ReasoningFactor(
                "Decision",
...
def create_policy_engine(
    ...
    return PolicyEngine(
        default_risk_config=risk_config,
        verbose=verbose,
