...
def evaluate_and_decide(
    ...
    try:
        ...
        if verbose:
            ...
            logger.info(
                f"  Position: {position_state.side.value if hasattr(position_state.side, 'value') else position_state.side}"
            )
        ...
        if verbose:
            ...
            logger.info(
                f"[Integration] Reasoning factors: {len(combined_reasoning['policy'])} policy, {len(combined_reasoning['eval'])} eval"
            )
...
def create_integrated_evaluator_factory(
    use_causal_policy: bool = False, **kwargs
) -> callable:
    """
...
if __name__ == "__main__":
    ...
    position_state = PositionState(
        side=PositionSide.FLAT, size=0.0, entry_price=None, current_price=1.0850
    )
    ...
    print(
        f"Reasoning factors: {len(result['reasoning']['eval'])} eval + {len(result['reasoning']['policy'])} policy"
    )
    print("=" * 70 + "\n")
