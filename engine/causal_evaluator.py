...
try:
    from engine.scenario_simulator import ScenarioSimulator, ScenarioResult
...
class EvaluationResult:
    ...
    session_name: str = ""
    session_modifiers: Optional[Dict[str, float]] = None  # vol_scale, liq_scale, risk_scale
    flow_signals: Dict[str, Any] = field(default_factory=dict)  # stop_run, initiative, vwap_dist, etc
    ...
    regime_features: Dict[str, Any] = field(default_factory=dict)  # Feature breakdown from regime classifier
    regime_adjusted_eval: float = 0.0  # eval_score after regime conditioning
    regime_adjustments: Dict[str, float] = field(default_factory=dict)  # What was adjusted and by how much
    
    # Scenario Integration (v2.2)
    scenario_result: Optional[Any] = None  # ScenarioResult with regime-weighted scenarios
    scenario_ev: float = 0.0  # Expected value from scenarios
    scenario_confidence_boost: float = 0.0  # Confidence adjustment from scenario alignment
...
class CausalEvaluator:
    ...
    def evaluate(self, state: MarketState) -> EvaluationResult:
        ...
        if self.scenario_simulator and state.vwap and state.session_high and state.session_low:
    ...
    def _score_dealer(self, dealer_state: DealerState) -> ScoringFactor:
        ...
        """
        gamma_effect = -dealer_state.net_gamma_exposure * 0.4  # Positive gamma = mean revert
        
        spot_effect = dealer_state.net_spot_exposure * 0.3
        
        vega_effect = -dealer_state.vega_exposure * 0.2  # Short vega wants vol crush (slight up)
    ...
    def _score_time_regime(self, time_regime_state: TimeRegimeState) -> ScoringFactor:
        ...
        session_progress = time_regime_state.minutes_into_session / 960.0  # 16 hours = 960 min
    ...
    def _score_price_location(self, price_location_state: PriceLocationState) -> ScoringFactor:
        """
    ...
    def _compute_confidence(
        ...
        volatility_confidence = 1.0 - vol_extremity * 0.3  # Extreme vol = lower confidence
    ...
    def _validate_state(self, state: MarketState) -> None:
        ...
        required_components = [
            'macro_state', 'liquidity_state', 'volatility_state',
            'dealer_state', 'earnings_state', 'time_regime_state',
            'price_location_state', 'macro_news_state'
        ]
        
        for component in required_components:
            if not hasattr(state, component):
                raise ValueError(f"[OFFICIAL_TOURNAMENT] Missing component: {component}")
    ...
    def _apply_session_adjustments(
        ...
        if session == 'GLOBEX':
            ...
            if abs(eval_score) > 0.5:
                adjustment = -eval_score * 0.1  # encourage mean-reversion of overnight extremes
            ...
            if state.round_level_proximity is not None and state.round_level_proximity > 0.8:
    ...
    def _condition_for_range(
        ...
        if abs(result.eval_score) > 0.5:
            ...
            result.regime_adjusted_eval = np.clip(result.eval_score + fade_boost, -1.0, 1.0)
    ...
    def _condition_for_reversal(
        ...
        if abs(result.eval_score) < 0.4:
            ...
            result.regime_adjusted_eval = np.clip(result.eval_score + counter_boost, -1.0, 1.0)
    ...
    def _apply_scenario_integration(
        ...
        if state.session_high and state.session_low:
            range_val = state.session_high - state.session_low
            expected_move = range_val * 0.6  # Typically 60% of session range
        else:
            expected_move = state.current_price * 0.01 if state.current_price else 0.01
        
        # Generate scenarios with regime conditioning
        scenario_result = self.scenario_simulator.simulate_scenarios(
            current_price=state.current_price or 0.0,
            vwap=state.vwap or state.current_price or 0.0,
            session_high=state.session_high or 0.0,
            session_low=state.session_low or 0.0,
        ...
        if state.current_price and state.current_price > 0:
            result.scenario_ev = (scenario_result.expected_price - state.current_price) / state.current_price
        
        # Adjust confidence based on scenario alignment
        # High alignment → confidence boost, low alignment → confidence penalty
        confidence_adjustment = scenario_result.regime_alignment * 0.10 * result.regime_confidence
