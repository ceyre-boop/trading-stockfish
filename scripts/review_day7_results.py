"""Post-run review for PAPER Day 7.

Checks:
- storage/decisions has run_id=day7 rows
- storage/audits has run_id=day7 rows (>=1)
- storage/stats has run_id=day7 rows
- scheduler log for day7 present
- SAFE_MODE not indicated in scheduler log

Writes summary to logs/scheduled/daily_day7_review.log and exits non-zero on failure.
"""
...
LOG_PATH = LOG_DIR / "daily_day7_review.log"
RUN_ID = "day7"
...
def main() -> None:
    ...
    if audits_count < 1:
        errors.append("audits missing for day7 (expect >=1 row)")
    if stats_count < 1:
    ...
    if not sched_log:
        errors.append("scheduler log for day7 not found")
    else:
    ...
    if errors:
        print("FAIL: Day 7 review")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("PASS: Day 7 review")
