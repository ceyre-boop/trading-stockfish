"""Manual recovery run for PAPER day6.

Executes daily_run for day6 and verifies storage artifacts without overwriting.
Logs to logs/scheduled/daily_day6_recovery.log and prints PASS/FAIL summary.
"""
...
LOG_PATH = LOG_DIR / "daily_day6_recovery.log"
RUN_ID = "day6"
...
def main() -> None:
    ...
    if audits < 1:
        errors.append("audits missing for day6 (expect >=1)")
    if stats < 1:
        errors.append("stats missing for day6")

    if errors:
        print("FAIL: Day6 recovery")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("PASS: Day6 recovery")
