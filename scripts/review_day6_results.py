"""Post-run review for PAPER Day 6.

Checks:
- storage/decisions has run_id=day6 rows
- storage/audits has run_id=day6 rows (>=1)
- storage/stats has run_id=day6 rows
- scheduler log for day6 present
- SAFE_MODE not indicated in scheduler log

Writes summary to logs/scheduled/daily_day6_review.log and exits non-zero on failure.
"""
...
LOG_PATH = LOG_DIR / "daily_day6_review.log"
RUN_ID = "day6"
...
def main() -> None:
    ...
    if decisions_count < 1:
        errors.append("decisions missing for day6")
    if audits_count < 1:
        errors.append("audits missing for day6 (expect >=1 row)")
    if stats_count < 1:
        errors.append("stats missing for day6")

    sched_log = _find_scheduler_log()
    report["scheduler_log"] = str(sched_log) if sched_log else None
    if not sched_log:
        errors.append("scheduler log for day6 not found")
    else:
    ...
    if errors:
        print("FAIL: Day 6 review")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("PASS: Day 6 review")
