"""Post-run review for PAPER Day 3.

Checks:
- storage/decisions has run_id=day3 rows
- storage/audits has run_id=day3 rows (>=1)
- storage/stats has run_id=day3 rows
- logs/scheduled contains Day 3 scheduler log (daily_*.log with run_id day3)
- SAFE_MODE not indicated in logs

Writes summary to logs/scheduled/daily_day3_review.log and exits non-zero on failure.
"""
...
LOG_PATH = LOG_DIR / "daily_day3_review.log"
RUN_ID = "day3"
...
def main() -> None:
    ...
    if decisions_count < 1:
        errors.append("decisions missing for day3")
    if audits_count < 1:
        errors.append("audits missing for day3 (expect >=1 row)")
    if stats_count < 1:
        errors.append("stats missing for day3")

    sched_log = _find_scheduler_log()
    report["scheduler_log"] = str(sched_log) if sched_log else None
    if not sched_log:
        errors.append("scheduler log for day3 not found")
    else:
    ...
    if errors:
        print("FAIL: Day 3 review")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("PASS: Day 3 review")
