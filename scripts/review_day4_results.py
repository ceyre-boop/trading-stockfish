"""Post-run review for PAPER Day 4.

Checks:
- storage/decisions has run_id=day4 rows
- storage/audits has run_id=day4 rows (>=1)
- storage/stats has run_id=day4 rows
- scheduler log for day4 present
- SAFE_MODE not indicated in scheduler log

Writes summary to logs/scheduled/daily_day4_review.log and exits non-zero on failure.
"""
...
LOG_PATH = LOG_DIR / "daily_day4_review.log"
RUN_ID = "day4"
...
def main() -> None:
    ...
    if audits_count < 1:
        errors.append("audits missing for day4 (expect >=1 row)")
    if stats_count < 1:
    ...
    if not sched_log:
        errors.append("scheduler log for day4 not found")
    else:
    ...
    if errors:
        print("FAIL: Day 4 review")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("PASS: Day 4 review")
