"""Manual recovery run for PAPER day5.

Executes daily_run for day5 and verifies storage artifacts without overwriting.
Logs to logs/scheduled/daily_day5_recovery.log and prints PASS/FAIL summary.
"""
...
LOG_PATH = LOG_DIR / "daily_day5_recovery.log"
RUN_ID = "day5"
...
def main() -> None:
    ...
    if audits < 1:
        errors.append("audits missing for day5 (expect >=1)")
    if stats < 1:
        errors.append("stats missing for day5")

    if errors:
        print("FAIL: Day5 recovery")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("PASS: Day5 recovery")
