...
class DataIntegrityLogger:
    ...
    def __init__(self, log_dir: str = "logs/data_integrity"):
        ...
        self.log_file = os.path.join(
            log_dir,
            f"data_integrity_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def log_anomaly(self, anomaly_type: str, description: str, severity: str = "WARNING"):
        ...
        self.anomalies.append({'type': anomaly_type, 'description': description, 'severity': severity})
...
class DataIntegrityLayer:
    ...
    def verify_time_causality(
        ...
        self.logger.log_check("TimestampUniqueness", "PASS", f"{len(df)} unique, sorted timestamps")
        ...
        rolling_cols = [col for col in feature_columns if any(x in col.lower() for x in ['atr', 'vol', 'sma', 'ema', 'rolling'])]
        ...
        if rolling_cols:
            for col in rolling_cols:
                if df[col].isna().sum() > len(df) * 0.5:
                    ...
                    self.logger.log_check(f"Rolling_{col}", "PASS", f"Properly aligned (NaN: {df[col].isna().sum()} rows)")
        ...
        for col in feature_columns:
            if col in df.columns and df[col].dtype in ['float64', 'int64']:
                ...
                if value_repeats > len(df) * 0.1:
                    msg = f"Column {col} has suspicious repeating values (>10% identical)"
                    self.logger.log_anomaly("ForwardFill", msg, "WARNING")
                else:
                    self.logger.log_check(f"ForwardFill_{col}", "PASS", "No suspicious repeats")
    ...
    def verify_no_future_joins(
        ...
        if macro_df is not None and not macro_df.empty:
            ...
            if macro_ts_col is None:
                raise DataIntegrityError("Could not identify timestamp column in macro_df")
        ...
        if news_df is not None and not news_df.empty:
            ...
            if news_ts_col is None:
                raise DataIntegrityError("Could not identify timestamp column in news_df")
            
            market_ts = pd.to_datetime(market_df[market_ts_col]).sort_values()
            news_ts = pd.to_datetime(news_df[news_ts_col])
            
            self.logger.log_check("NewsJoins", "PASS",
    ...
    def verify_no_asof_fields(self, df: pd.DataFrame) -> bool:
        ...
        self.logger.log_check("AsOfFields", "PASS", f"No suspicious fields in {len(df.columns)} columns")
        return True
    
    def verify_monotonic_timestamps(self, df: pd.DataFrame, timestamp_column: str = 'timestamp') -> bool:
        """
        ...
        if df.empty:
            self.logger.log_check("MonotonicTimestamps", "PASS", "Empty dataframe (trivially monotonic)")
            return True
        ...
        if huge_gaps > 0:
            self.logger.log_anomaly("TimestampGaps", f"Found {huge_gaps} gaps >24 hours", "INFO")
        
        self.logger.log_check("MonotonicTimestamps", "PASS", 
    ...
    def verify_dataset_cleanliness(
        ...
        try:
            ...
            if feature_columns is None:
                feature_columns = [col for col in market_df.columns if col != 'timestamp']
