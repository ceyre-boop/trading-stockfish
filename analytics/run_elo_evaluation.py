...
from analytics.elo_engine import (
    Trade, TradeType, evaluate_engine, 
    PerformanceMetrics, Rating
)
from analytics.data_loader import (
    DataLoader, MarketStateBuilder, MarketState,
    validate_data
)
...
class DebugCausalLogger:
    ...
    def __init__(self, symbol: str, timeframe: str):
        ...
        self.log_file = logs_dir / f'debug_causal_run_{symbol}_{timeframe}_{timestamp}.log'
...
"""
    ...
    def log_candle(self, timestamp: str, ohlcv: Dict, eval_result: Dict, 
                   decision: Dict, position_state: Dict):
    ...
    def generate_candles(
        ...
        for _ in range(num_candles - 1):
            ...
            price_change = prices[-1] * (
                self.drift * dt + 
                self.volatility * np.sqrt(dt) * random_shock
            )
...
class TradingEngineSimulator:
    ...
    def run_simulation(self) -> List[Trade]:
        ...
        for idx in range(max(sma_fast, sma_slow), len(self.price_data)):
            ...
            if self.track_health and self.health_monitor and idx > max(sma_fast, sma_slow):
            ...
            health_status = self.health_monitor.get_health_status() if self.track_health else "HEALTHY"
...
class RealDataTradingSimulator:
    ...
    def __init__(
        ...
        if self.execution_simulator is None:
            try:
                ...
                logger.warning(f"Could not initialize ExecutionSimulator: {e}. Using mid-price fills.")
    ...
    def run_simulation(self) -> List[Trade]:
        ...
        logger.info(f"Running trading simulation on {len(self.price_data)} real data candles...")
                ...
                if self.debug_logger and i < len(self.price_data):
                    ...
                    eval_result = {
                        ...
                        'factors': {
                            'trend': 0.5,
                            'momentum': 0.5,
                            'volatility': market_state.volatility.vol_magnitude if market_state else 0.5,
                            'volume': market_state.liquidity.confidence if market_state else 0.5,
                            'mean_reversion': 0.5,
                            'sentiment': 0.5,
                            'liquidity': market_state.liquidity.confidence if market_state else 0.5,
                            'regime': 0.5
                        },
                        'regime': 'BULL' if decision and decision.get('action') == 'BUY' else 'BEAR'
                    }
                    
                    decision_log = {
                        'action': decision.get('action', 'HOLD') if decision else 'HOLD',
                        'conviction_zone': 'MODERATE_SIGNAL' if decision else 'NO_SIGNAL',
                        'reasoning': decision.get('reason', '') if decision else 'No signal'
                    }
                    
                    position_state = {
                        'position_size': len(self.open_positions),
                        'entry_price': 0,
                        'risk_metrics': {'stop_loss': 0, 'take_profit': 0, 'risk_reward': 0}
                    }
                    
                    self.debug_logger.log_candle(timestamp, ohlcv, eval_result, decision_log, position_state)
                
                if decision:
                    self._execute_decision_on_real_data(
                        decision,
                        market_state,
                        i
                    )
    ...
    def _make_decision_from_state(self, market_state: MarketState) -> Optional[Dict[str, Any]]:
        ...
        if (market_state.liquidity.liquidity_type.value == 'abundant' and 
            market_state.volatility.vol_state.value in ['low', 'normal']):
...
class RealDataTournament:
    ...
    def __init__(
        ...
        if official_mode:
            ...
            logger.warning("[OFFICIAL TOURNAMENT MODE] Real data ONLY. Synthetic paths DISABLED.")
    ...
    def run(self) -> Tuple[Rating, Dict[str, Any]]:
        ...
        if self.verbose:
            ...
            if trades:
                ...
                print(f"  [OK] Winning trades: {winning_trades}/{len(trades)} ({winning_trades/len(trades)*100:.1f}%)")
        ...
        if self.debug_logger:
            summary = {
                ...
                'winning_trades': sum(1 for t in trades if t.pnl_points > 0),
                'win_rate': sum(1 for t in trades if t.pnl_points > 0) / len(trades) if trades else 0,
                'total_return': getattr(rating.metrics, 'total_return', 0) if rating.metrics else 0,
                'sharpe_ratio': getattr(rating.metrics, 'sharpe_ratio', 0) if rating.metrics else 0,
                'max_drawdown': getattr(rating.metrics, 'max_drawdown', 0) if rating.metrics else 0
            }
    ...
    def _load_and_prepare_data(self) -> Tuple[pd.DataFrame, List[MarketState]]:
        ...
        if self.data_path.endswith('.csv'):
            ...
            price_data = loader.load_parquet(self.data_path, self.symbol, self.timeframe)
        else:
        ...
        if 'bid' not in price_data.columns or 'ask' not in price_data.columns:
            price_data = loader.estimate_spreads(price_data, self.symbol, self.timeframe)
    ...
    def _prepare_results(
        ...
        results = {
            'tournament_info': {
                ...
                'mode': 'official_tournament' if self.official_mode else 'real_data_tournament',
                'lookahead_safe': True,  # GUARANTEED: Time-causal, no future leakage
                'causal_eval': self.causal_evaluator is not None,  # NEW: Causal eval flag
            ...
            'elo_rating': {
                ...
                'strength_class': rating.strength_class.value if rating.strength_class else 'Unknown',
            ...
            'trade_statistics': {
                ...
                'win_rate': sum(1 for t in trades if t.pnl_points > 0) / len(trades) * 100 if trades else 0
            }
        }
    ...
    def _display_results(self, rating: Rating, results: Dict[str, Any]):
        ...
        print(f"  Data Source:         {info['data_source'].upper()} (verified historical)")
        ...
        print(f"  Baseline Performance:        {comp_scores['baseline_performance']:.1%}")
        print(f"  Stress Test Resilience:      {comp_scores['stress_test_resilience']:.1%}")
        print(f"  Monte Carlo Stability:       {comp_scores['monte_carlo_stability']:.1%}")
        print(f"  Regime Robustness:           {comp_scores['regime_robustness']:.1%}")
        print(f"  Walk-Forward Efficiency:     {comp_scores['walk_forward_efficiency']:.1%}")
        print()
    ...
    def _save_results(self, rating: Rating, results: Dict[str, Any]):
        ...
        try:
            output_data = {
                **results,
                'rating_details': rating.to_dict() if hasattr(rating, 'to_dict') else {}
            }
...
def run_real_data_tournament(
    ...
    if official_mode:
        logger.warning("[OFFICIAL TOURNAMENT] Real-data-only mode ENABLED. Synthetic paths DISABLED.")
    if causal_evaluator is not None:
    ...
    logger.info(f"Tournament complete: {rating.strength_class.value} ELO {rating.elo_rating:.0f}")
    if official_mode:
        logger.info("[OFFICIAL] Results are time-causal, lookahead-safe, real-data only")
    if causal_evaluator is not None:
        logger.info("[CAUSAL] Evaluation completed with Stockfish-style deterministic scoring")
...
class ELOEvaluationRunner:
    ...
    def run(self) -> Rating:
        ...
        if self.verbose:
            print(f"\n{'='*70}")
            print(f"ELO EVALUATION PIPELINE - {'REAL DATA' if self.real_data else 'SYNTHETIC'}")
            print(f"{'='*70}")
            print(f"Symbol: {self.symbol}")
            if self.real_data:
        ...
        if self.verbose:
            print(f"  [OK] Loaded {len(price_data)} candles")
            print(f"  [OK] Price range: {price_data['close'].min():.4f} - {price_data['close'].max():.4f}")
            if market_states:
                print(f"  [OK] Reconstructed {len(market_states)} market states")
            print()
    ...
    def _load_real_data(self) -> Tuple[pd.DataFrame, List[MarketState]]:
        ...
        # Filter by date range if specified
        price_data = price_data[
            (price_data.index >= self.start_date) &
            (price_data.index <= self.end_date)
        ]
        
        if len(price_data) == 0:
            raise ValueError(f"No data found for date range {self.start_date} to {self.end_date}")
        ...
        if rating.stress_test_results:
            ...
            for result in rating.stress_test_results[:7]:
                if hasattr(result, 'scenario') and hasattr(result, 'degradation'):
                    print(f"  {result.scenario:.<30} {(1-result.degradation):.1%} retained")
            print()
...
class BrutalTournament:
    ...
    REGIMES = {
        ...
        'low_volatility': {'threshold': 0.005, 'description': 'Daily volatility < 0.5%'},
    ...
    STRESS_TESTS = {
        ...
        'correlation_breakdown': {'name': 'Correlation Breakdown', 'severity': 'moderate'},
        'trend_reversal': {'name': 'Trend Reversal', 'severity': 'moderate'}
    }
    
    def __init__(self, start_year: int = 2018, end_year: int = 2024, verbose: bool = True):
        ...
        if self.verbose:
            ...
            print(f"[BRUTAL TOURNAMENT] Symbols: {', '.join(self.DEFAULT_SYMBOLS.keys())}")
        ...
        for symbol, config in available_symbols.items():
            ...
            for year in range(self.start_year, self.end_year + 1):
                ...
                try:
                    ...
                    print(f"      [OK] {year}: Rating {rating.elo_rating:.0f} | Win% {results['trade_statistics']['win_rate']:.1f}%")
        ...
        for symbol in ['ES', 'NQ', 'EURUSD']:
            ...
            for year in range(self.start_year, self.end_year + 1):
                ...
                self.results[symbol][year] = {
                    ...
                    'strength_class': 'Master' if rating > 1600 else 'Expert' if rating > 1400 else 'Intermediate',
                    'results': {
                        'trade_statistics': {
                ...
                print(f"      [{symbol}] {year}: Rating {rating:.0f} | Win% {win_rate*100:.1f}%")
    ...
    def _analyze_regimes(self):
        ...
        strong = [k for k, v in regime_performance.items() if v['classification'] == 'strong']
        weak = [k for k, v in regime_performance.items() if v['classification'] == 'weak']
...
"""
        ...
        for symbol, yearly_results in self.results.items():
            total_trades = sum(
                yr_data.get('results', {}).get('trade_statistics', {}).get('total_trades', 0)
                for yr_data in yearly_results.values()
            )
            avg_rating = sum(yr_data.get('rating', 0) for yr_data in yearly_results.values()) / len(yearly_results) if yearly_results else 0
            
            win_rates = []
            for yr_data in yearly_results.values():
                wr = yr_data.get('results', {}).get('trade_statistics', {}).get('win_rate', 0)
                win_rates.append(wr)
            avg_win_rate = sum(win_rates) / len(win_rates) * 100 if win_rates else 0
            
            status = 'PASS' if avg_rating >= 1500 else 'REVIEW' if avg_rating >= 1300 else 'FAIL'
...
"""
        ...
        for symbol, yearly_results in self.results.items():
            ...
            for year in sorted(yearly_results.keys()):
                ...
                report += f"| {year} | {total_trades} | {win_rate:.1f}% | {rating:.0f} | |\n"
...
"""
    ...
    def _generate_failure_modes_report(self) -> str:
        ...
        report = """# CURRENT ENGINE FAILURE MODES
  ...
  python analytics/run_elo_evaluation.py --real-data --data-path data/ES_1h.csv --symbol ES --timeframe 1h --output results.json
    ...
    try:
        ...
        if args.brutal_tournament:
            ...
            args.verbose = True
            print("[BRUTAL TOURNAMENT] Cascading settings: real-tournament=True, official-tournament=True, causal-eval=True, verbose=True")
            print("[BRUTAL TOURNAMENT] Initializing multi-year, multi-symbol stress test...")
            ...
            try:
                ...
                brutal.run()
                print("[BRUTAL TOURNAMENT] ✅ Stress test complete. Results in analytics/brutal_runs/")
                print("[BRUTAL TOURNAMENT] ✅ Summary: BRUTAL_TOURNAMENT_SUMMARY.md")
                print("[BRUTAL TOURNAMENT] ✅ Analysis: CURRENT_ENGINE_FAILURE_MODES.md")
                return 0
            except Exception as e:
        ...
        if args.debug_causal_run:
            ...
            args.causal_eval = True
            args.verbose = True
            print("[DEBUG CAUSAL RUN] Cascading settings: real-tournament=True, official-tournament=True, causal-eval=True, verbose=True")
        
        # Validate real-data arguments
        if args.real_data and not args.data_path:
            print("[ERROR] --data-path is required when using --real-data", file=sys.stderr)
            return 1
        
        # Validate real-tournament arguments
        if args.real_tournament and not args.data_path:
            print("[ERROR] --data-path is required when using --real-tournament", file=sys.stderr)
            return 1
        
        # Validate --causal-eval usage (only with real tournaments)
        if args.causal_eval and not args.real_tournament and not args.official_tournament:
            print("[ERROR] --causal-eval requires either --real-tournament or --official-tournament", file=sys.stderr)
            return 1
        ...
        if args.causal_eval:
            try:
                ...
                causal_evaluator = CausalEvaluator(
                    verbose=args.verbose,
                    official_mode=args.official_tournament
                )
                print("[CAUSAL EVAL] CausalEvaluator initialized (Stockfish-style evaluation enabled)")
            except ImportError:
                print("[ERROR] CausalEvaluator not available. Check engine/causal_evaluator.py", file=sys.stderr)
                return 1
            except Exception as e:
                print(f"[ERROR] Failed to initialize CausalEvaluator: {e}", file=sys.stderr)
                return 1
        
        # Initialize debug logger if requested
        debug_logger = None
        if args.debug_causal_run:
            debug_logger = DebugCausalLogger(symbol=args.symbol, timeframe=args.timeframe)
            print(f"[DEBUG] Debug logger initialized for {args.symbol} ({args.timeframe})")
        
        # Validate official-tournament arguments (stricter)
        if args.official_tournament:
            if not args.data_path:
                print("[ERROR] [OFFICIAL TOURNAMENT] --data-path is REQUIRED", file=sys.stderr)
                return 1
            if not os.path.exists(args.data_path):
                print(f"[ERROR] [OFFICIAL TOURNAMENT] Data file not found: {args.data_path}", file=sys.stderr)
                return 1
