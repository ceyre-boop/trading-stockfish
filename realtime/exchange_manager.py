...
from engine.connector_health import ConnectorHealthMonitor, propagate_health_events
...
class ExchangeManager:
    ...
    def __init__(
        self, failover_strategy: FailoverStrategy = FailoverStrategy.BEST_AVAILABLE
    ):
        """
        ...
        self.running = False
        self.health_monitor: Optional[ConnectorHealthMonitor] = None
        self._guardrail_callback = None
        self._safe_mode_callback = None
        self._anomaly_callback = None
        self._health_eval_interval = 1.0
        self._health_live_mode = True
    ...
    def add_connector(
        ...
        if self.health_monitor:
            connector.health_monitor = self.health_monitor
        ...
        logger.info(
            f"Added {name} connector (primary={primary}, data_only={data_only})"
        )
    ...
    def attach_health_monitor(
        ...
        safe_mode_callback=None,
        anomaly_callback=None,
        enable_safe_mode: bool = True,
        evaluation_interval: float = 1.0,
    ) -> None:
        """Attach Phase 11 connector health monitor to all connectors."""

        self.health_monitor = health_monitor
        self._guardrail_callback = guardrail_callback
        self._safe_mode_callback = safe_mode_callback if enable_safe_mode else None
        self._anomaly_callback = anomaly_callback
        self._health_eval_interval = max(0.5, float(evaluation_interval))
        self._health_live_mode = bool(enable_safe_mode)
        self.health_check_interval = min(
            self.health_check_interval, self._health_eval_interval
        )

        for connector in self.connectors.values():
            connector.health_monitor = health_monitor
    ...
    def start_all(self) -> bool:
        ...
        logger.info(
            f"ExchangeManager: {success_count}/{len(self.connectors)} connectors started"
        )
    ...
    def subscribe_price(self, symbols: List[str], connectors: List[str] = None) -> bool:
        """
        ...
        for symbol in symbols:
            for conn_name in connectors:
                ...
                try:
                    ...
                    logger.error(
                        f"Subscription error for {symbol} on {conn_name}: {str(e)}"
                    )

        return success

    def subscribe_orderbook(
        self, symbols: List[str], depth: int = 10, connectors: List[str] = None
    ) -> bool:
    ...
    def unsubscribe(self, symbols: List[str], connectors: List[str] = None):
    ...
    def get_stats(self) -> Dict:
        ...
        for name in self.connector_order:
            ...
            stats["total"]["updates_received"] += connector_stats.get(
                "updates_received", 0
            )
            stats["total"]["updates_normalized"] += connector_stats.get(
                "updates_normalized", 0
            )
            stats["total"]["orders_submitted"] += connector_stats.get(
                "orders_submitted", 0
            )
            stats["total"]["orders_filled"] += connector_stats.get("orders_filled", 0)
            stats["total"]["orders_cancelled"] += connector_stats.get(
                "orders_cancelled", 0
            )
    ...
    def _select_execution_connector(self) -> Optional[BaseConnector]:
        ...
        candidates = [
            ...
            for name in self.execution_connectors
        ...
        if self.failover_strategy == FailoverStrategy.PRIMARY_BACKUP:
            ...
            if self.primary_connector and self.primary_connector in [
            ...
            best = max(
                candidates,
                key=lambda c: (c.is_connected, -c.stats.get("data_errors", 0)),
            )
            return best
    ...
    def __str__(self):
        ...
        status = ", ".join(
            [f"{name}:{c.get_status()}" for name, c in self.connectors.items()]
        )
