...
from .safety_layer import SafetyLayer, SafetyEvent, SafetyEventType
...
class LiveTradingOrchestrator:
    ...
    def __init__(
        ...
        self._failsafe_degradation_count = self.config.get('failsafe_degradation_count', 3)
    ...
    def _validate_state_transition(
        ...
        valid_transitions = {
            OrchestratorState.STARTUP: [OrchestratorState.READY, OrchestratorState.SHUTDOWN],
            OrchestratorState.READY: [OrchestratorState.LIVE, OrchestratorState.SHUTDOWN],
            OrchestratorState.LIVE: [OrchestratorState.DEGRADED, OrchestratorState.SHUTDOWN],
            OrchestratorState.DEGRADED: [OrchestratorState.LIVE, OrchestratorState.FAILSAFE, OrchestratorState.SHUTDOWN],
            OrchestratorState.FAILSAFE: [OrchestratorState.READY, OrchestratorState.SHUTDOWN],
    ...
    def _assess_overall_health(self) -> None:
        ...
        try:
            ...
            if all_connectors_down and current_state in [OrchestratorState.LIVE, OrchestratorState.READY]:
                self.logger.critical("All connectors disconnected - entering DEGRADED state")
            ...
            if critical_failures > 1 and current_state == OrchestratorState.LIVE:
                self.logger.critical("Multiple connector failures - entering DEGRADED state")
                self.stats.connector_failures += 1
                self._set_state(OrchestratorState.DEGRADED)
                self._consecutive_degradations += 1
        ...
        try:
            if not event.is_connected:
                self.logger.warning(
                    ...
                    event.connector_name, event.latency_ms, event.error_count
                )
                self.stats.connector_failures += 1
            else:
                self.logger.debug(
                    "Connector %s healthy: latency=%.1fms, messages=%d",
                    event.connector_name, event.latency_ms, event.messages_received
                )
        ...
        try:
            if event.error_count > 0:
                self.logger.warning(
                    ...
                    event.error_count, event.latency_p99_ms
                )
    ...
    def _handle_safety_event(self, event: SafetyEvent) -> None:
        ...
        try:
            ...
            self.logger.warning(
                ...
                event.event_type.value, event.severity, event.message
            )
    ...
    def _trigger_failsafe(self) -> None:
        ...
        """
        try:
            self.logger.critical("FAILSAFE TRIGGERED - Flattening positions and disabling trading")
    ...
    def register_state_change_callback(self, callback: Callable[[OrchestratorState, OrchestratorState], None]) -> None:
        """Register callback for state changes."""
        self._on_state_change_callbacks.append(callback)
    
    def register_safety_event_callback(self, callback: Callable[[SafetyEvent], None]) -> None:
        """Register callback for safety events."""
        self._on_safety_event_callbacks.append(callback)
    
    def register_health_degradation_callback(self, callback: Callable[[str], None]) -> None:
