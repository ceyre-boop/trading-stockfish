...
class IBKRConnector(BaseConnector):
    ...
    def __init__(self, router=None, host='127.0.0.1', port=7497, client_id=1):
        ...
        logger.info(f"Initialized IBKR connector ({host}:{port}, client_id={client_id})")
    ...
    def send_order(self, order: Order) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"IBKR: Submitted {order.side.value} {order.quantity} {order.symbol} @ {order.price}")
