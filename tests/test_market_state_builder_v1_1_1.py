...
def create_sample_ohlcv():
    ...
    dates = pd.date_range('2026-01-15 03:00:00', periods=100, freq='1T', tz=timezone.utc)
    df = pd.DataFrame({
...
def test_market_state_builder_session_integration():
    ...
    for state in states:
        assert 0.5 <= state.session_vol_scale <= 2.0, f"Volatility scale out of range: {state.session_vol_scale}"
        assert 0.5 <= state.session_liq_scale <= 2.0, f"Liquidity scale out of range: {state.session_liq_scale}"
        assert 0.5 <= state.session_risk_scale <= 2.0, f"Risk scale out of range: {state.session_risk_scale}"
